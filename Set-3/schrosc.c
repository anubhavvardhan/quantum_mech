/* SCHROSC.C  *.MEXW32 the action is described in SCHROSC.M
		 
 Calling syntax:

			[Psi_2, t_2, V] = SCHROSC(Psi_1, t_1, Vo, DV, Par);
   with:
...
INPUT QUANTITIES:
	...
	SCHROSC.DLL solves the Schroedinger Eq. for a 1-Dim wave function Psi
   ...
   where dt is elementary the time interval passed by Par(1); t_1
   is the initial time; omega, passed by Par[2], is the radian
	frequency of the perturbation in radian/femtosec; cycleNum,
	passed by Par(3), is the number of time the computation is carried
	out at each call.

  	Author: Renato Nobili - Dept. of Physics of Padova University, October 2010
*/
...
/*
	...
	1 electronVolt. 
	V(x,t) = Vo(x) + DV2(x)*tanh[(t-to)/Dt], where t=Tnum*dt 
...
void SCHROSC(Complex *Psi, double *to, double *Vo, double *DV,
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  {	
	...
	mexPrintf("	PAR = [dt, omega, cycleNum, xWidth, m_r];\n");
  	...
  	mexPrintf("	omega = perturbation radian frequency in femtosec^-1 [e.g. 10];\n");
   ...
   mexPrintf("	xWidth = x-axis interval in Angstrom [e.g. 20];\n");
  	...
  	mexPrintf("	Call 'help SCHROSC' from MATLAB command-line for other instructions.\n\n");
	...
	mexErrMsgTxt("\n	(MexFile SCHROSC.MEXW32 requires 4 input arguments)");  
  }
  else if(nlhs > 3)
  {
		mexErrMsgTxt("MexFile SCHROSC.MEXW32 accepts at most three output argument.");
  };
 ...
 /* Assign pointers to the various parameters */
  ...
  {
	   mexErrMsgTxt("MexFile SCHROSC.MEXW32 requires that the first argument be a complex vector.");
  };
...
/* Check the dimensions of T_IN.  T_IN is a real number. */
  ...
  if(mxIsSparse(T_IN)||!mxIsNumeric(T_IN)|| 
		!mxIsDouble(T_IN)|| mt*nt != 1)
  {
	   mexErrMsgTxt("MexFile SCHROSC.MEXW32 requires that second argument\n is a real number.");
  };
 ...
 /* Assign pointer to second argument data */
  ...
  {
	   mexErrMsgTxt("MexFile SCHROSC.MEXW32 requires that first,third and fourth argument\n have same dimension.");
  };
  ...
  if(!mxIsNumeric(PAR)||!mxIsDouble(PAR)||min(mpar,npar)!=1 || 
		max(mpar, npar)!=5)
  {
	    mexErrMsgTxt("MexFile SCHROSC.MEXW32 requires that fifth argument be a 5-dim real vector.");
  };
...
/* [dt, Dt,  cycleNum, xWidth, m_r] */
 ...
 /* Do the actual computations by the subroutine:
   SCHROSC(Complex *Psi, double *t_1, double *Vo, double *DV,
	double dt, double Dt, int cycleNum, double dx, double m_r, 
   int N, double *V) */ 

 	SCHROSC(Psi, &to, Vo, DV, Par[0], Par[1], cycleNum, dx, Par[4], N, V);  
